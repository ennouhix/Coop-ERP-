...
import { EMPTY_LINE, EMPTY_SALES_ORDER_FORM, type SalesOrderCreateValues } from "./types";
...
export function SalesOrderCreatePage() {
  ...
  const [values, setValues] = useState<SalesOrderCreateValues>(EMPTY_SALES_ORDER_FORM);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (values.lines.length === 0 || values.lines.some((l) => !l.product_id || !l.quantity_ordered)) {
      setError(t("sales.error_lines_required"));
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="font-display text-2xl font-bold text-ink-900">{t("sales.new")}</h1>

      <form onSubmit={handleSubmit} className="mt-6 space-y-5 rounded-lg border border-ink-900/5 bg-white p-6 shadow-sm">
        <div className="grid grid-cols-2 gap-4">
          <SelectField
            id="customer_id" label={t("sales.field.customer")} required
            value={values.customer_id} onChange={(e) => setValues((p) => ({ ...p, customer_id: e.target.value }))}
          >
            <option value="">{t("catalog.select_placeholder")}</option>
            {customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </SelectField>
          <SelectField
            id="warehouse_id" label={t("sales.field.warehouse")} required
        ...
        <div className="grid grid-cols-2 gap-4">
          <TextField
            id="order_date" type="date" label={t("sales.field.order_date")} required
            value={values.order_date} onChange={(e) => setValues((p) => ({ ...p, order_date: e.target.value }))}
          />
          <TextField
            id="expected_delivery_date" type="date" label={t("sales.field.expected_delivery_date")}
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm font-medium text-ink-800">{t("sales.lines_title")}</span>
            <Button type="button" variant="secondary" onClick={addLine}>
              <Plus className="h-4 w-4" />{t("sales.add_line")}
            </Button>
          </div>
          ...
          <div className="space-y-2">
            {values.lines.map((line, index) => (
              <div key={index} className="grid grid-cols-[1fr,110px,110px,32px] items-end gap-2 rounded-md border border-ink-900/10 p-3">
                <SelectField
                  id={`line_product_${index}`} label={t("sales.field.product")}
                ...
                <TextField
                  id={`line_qty_${index}`} type="number" min={0.001} step="0.001" label={t("sales.field.quantity")}
                  value={line.quantity_ordered} onChange={(e) => updateLine(index, "quantity_ordered", e.target.value)}
                />
                <TextField
                  id={`line_price_${index}`} type="number" min={0} step="0.01" label={t("sales.field.unit_price")}
          ...
          <p className="mt-3 text-end text-sm font-semibold text-ink-900">
            {t("sales.total")} : {formatMoney(total)}
          </p>
        </div>

        <TextareaField
          id="notes" label={t("sales.field.notes")}
