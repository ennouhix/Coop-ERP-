...
def _line_amount_sum(
    queryset, quantity_field: str = "quantity", price_field: str = "unit_price"
) -> Decimal:
...
def get_dashboard_summary(
    *, cooperative: Cooperative, date_from: date | None = None, date_to: date | None = None
) -> dict:
    if date_to is None:
        date_to = date.today()
...
def _members_summary(cooperative: Cooperative) -> dict:
    return {
        "active_count": Member.objects.filter(
            cooperative=cooperative, status=MemberStatus.ACTIVE
        ).count(),
...
def _partners_summary(cooperative: Cooperative) -> dict:
    return {
        "active_customers": Partner.objects.filter(
            cooperative=cooperative, is_customer=True
        ).count(),
        "active_suppliers": Partner.objects.filter(
            cooperative=cooperative, is_supplier=True
        ).count(),
    }
...
def _sales_summary(cooperative: Cooperative, date_from: date, date_to: date) -> dict:
    ...
    return {
        ...
        "orders_partially_delivered": orders.filter(
...
def _purchases_summary(cooperative: Cooperative, date_from: date, date_to: date) -> dict:
    ...
    return {
        ...
        "orders_partially_received": orders.filter(
...
def _stock_summary(cooperative: Cooperative) -> dict:
    ...
    total_value = levels.annotate(_value=value_expr).aggregate(total=Sum("_value"))[
        "total"
    ] or Decimal("0")
