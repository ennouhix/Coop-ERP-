...
class AuditLogTestCase(APITestCase):
    ...
    def test_login_creates_audit_entry(self) -> None:
        self.client.post(
            ...
            {"email": "owner@argane.ma", "password": "MotDePasseSolide123"},
        )
        self.assertTrue(
            AuditLog.objects.filter(cooperative=self.cooperative, action="user.login").exists()
        )
    ...
    def test_failed_login_does_not_create_audit_entry(self) -> None:
        self.client.post(
            reverse("authentication:login"),
            {"email": "owner@argane.ma", "password": "MauvaisMotDePasse"},
        )
    ...
    def test_deactivation_creates_audit_entry(self) -> None:
        ...
        self.assertTrue(
            AuditLog.objects.filter(
                action="user.deactivated", target_id=str(self.staff.id)
            ).exists()
        )
    ...
    def test_stock_transfer_creates_audit_entry(self) -> None:
        second_warehouse = Warehouse.objects.create(
            cooperative=self.cooperative, code="WH-0002", name="Entrepôt 2"
        )
        inventory_services.record_stock_in(
            product=self.product, warehouse=self.warehouse, quantity=Decimal("50"), actor=self.admin
        )
    ...
    def test_purchase_order_lifecycle_creates_audit_entries(self) -> None:
        ...
        purchases_services.record_purchase_receipt(
            order=order,
            actor=self.admin,
            receipts=[{"line_id": line.id, "quantity": Decimal("20")}],
        )
    ...
    def test_invoice_payment_creates_audit_entry(self) -> None:
        ...
        billing_services.record_payment(
            invoice=invoice, amount=Decimal("200"), payment_date=date(2026, 7, 5), actor=self.admin
        )
